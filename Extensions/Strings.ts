interface String {
	 ...
	 */
	indexOf(searchString: string, start?: number, end?: number): number;
}
...
String.prototype.indexOf = function (searchString: string, start?: number, end?: number) {
	if (!start && !end) return this.indexOf(searchString);
	else if (start && !end) return this.indexOf(searchString, start);
	else if (!start && end) return this.substring(0, end).indexOf(searchString);
